...
#### The `LML_DEFINE_TYPE` macro
```C
#define LML_DEFINE_TYPE(PREFIX, TYPE, PREV_OPT, MEMSIZE_OPT, CLEARABLE_OPT, FIELDS...)
```
#### Example use: 
```C
LML_DEFINE_TYPE(myprefix, struct entry, LML_NO_OPT, LML_NO_OPT, {
    uint64_t timestamp;
    uint64_t data;
    // etc ...
});
...
The `LML_DEFINE_TYPE` generates code for a new log type. In order to do that, lmlogs creates a new type `struct entry` with fields `uint64_t timestamp` and `uint64_t data` which will be the primitive data stored by the log.
...
#### The `LML_DECLARE_TYPE` macro
```C
#define LML_DECLARE_TYPE(PREFIX, TYPE, PREV_OPT, MEMSIZE_OPT, CLEARABLE_OPT)
```
#### Example use:
```C
LML_DECLARE_TYPE(myprefix, struct entry, LML_NO_OPT, LML_NO_OPT);
```
